...
import 'package:intravel_ease/providers/result_search_distance_provider.dart';
...
class ResultSearchDistanceScreen extends StatefulWidget {
  const ResultSearchDistanceScreen({Key? key}) : super(key: key);

  @override
  State<ResultSearchDistanceScreen> createState() =>
      _ResultSearchDistanceScreenState();
}

class _ResultSearchDistanceScreenState
    extends State<ResultSearchDistanceScreen> {
  ResultSearchDistanceProvider initProvider = ResultSearchDistanceProvider();
  ...
  Widget build(BuildContext context) {
    ...
    return ChangeNotifierProvider.value(
      value: initProvider,
      child: Consumer<ResultSearchDistanceProvider>(
                    ...
                    itemBuilder: (context, index) => Padding(
                      ...
                      child: BoxSearchDistance(
                        ...
                        image: ApiUtil.urlBase +
                            'storage/' +
                            snapshot.data![index]!.image!.gambar_wisata_gambar
                                .toString(),
                        rating: snapshot.data?[index]?.wisata_rating,
                        topText: snapshot.data![index]!.wisata_nama.toString(),
                        midleText: snapshot.data![index]!.wisata_kota
                                .toString() +
                            ', ' +
                            snapshot.data![index]!.wisata_provinsi.toString(),
